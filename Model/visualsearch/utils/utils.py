...
def load_image(img_path, name, image_size='default'):
    ...
    if image_size != 'default':
        img = img_as_ubyte(transform.resize(img, (768,1024)))

    return img

def save_csv_heatmap(output_folder,file_name,data):
    if not path.exists(output_folder):
        makedirs(output_folder)
    pd.DataFrame(data).to_csv(path.join(output_folder,file_name),index=False, header=False)
...
def add_scanpath_to_dict(image_name, image_scanpath, target_bbox_in_grid, target_object, grid, config, dataset_name, dict_,memory_set):
    target_found = image_scanpath['target_found']
    scanpath_x   = image_scanpath['scanpath_x']
    scanpath_y   = image_scanpath['scanpath_y']
    searched_object_indexes = image_scanpath['searched_object_indexes']
    ...
    dict_[image_name] = {'subject' : 'nnIBS Model', 'dataset' : dataset_name, 'image_height' : int(grid.size()[0]), 'image_width' : int(grid.size()[1]), \
        'receptive_height' : 1, 'receptive_width' : 1, 'target_found' : target_found, 'target_bbox' : target_bbox_in_grid, \
                 'X' : list(map(int, scanpath_x)), 'Y' : list(map(int, scanpath_y)),'elm_info_gained':list(map(float,image_scanpath["elm_info_gained"])), 'searched_object_indexes':list(map(int, searched_object_indexes)),'memory_set':memory_set, 'target_object' : target_object, 'max_fixations' : config['max_saccades'] + 1
        }
...
def get_target_bbox(trial, image_name, grid,image_size, model_image_size):
    ...
    if "target_matched_row" in trial:
        ...
        target_bbox      = [rescale_coordinate(target_bbox[i], image_size[i % 2 == 1], model_image_size[i % 2 == 1]) for i in range(len(target_bbox))]

        # Convert target bounding box to grid cells
        target_bbox_in_grid = np.empty(len(target_bbox), dtype=np.int32)
        ...
        if not(are_within_boundaries((target_bbox_in_grid[0], target_bbox_in_grid[1]), (target_bbox_in_grid[2], target_bbox_in_grid[3]), np.zeros(2), grid.size())):
    ...
    return target_bbox_in_grid, next_iteration

def get_coordinates(fixations, axis):
